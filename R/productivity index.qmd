...
```{r fig-dist-biomass}
#| code-fold: true
#| fig.cap: "Variation in total biomass. Each point is a 2 m radii circle. Units unknown."
...
We see in @fig-biomassInc that biomass accumulated relatively linearly for Telemakr and Trøndelag, but that for the more productive region Hedmark-Akerhus we see a plateu caused by more trees going into the 7th height category where all trees are said to be 325 cm even if they are much taller in reality.
...
dat_site <- dat_site |>
  group_by(LocalityCode) |>
  mutate(lowestYear = case_when(
    Biomass_norm == 0 ~ yearsSinceStart),
    lowestYear = max(lowestYear, na.rm=T)) |>
  filter(Biomass_norm > 25000) |>
  slice_min(Biomass_norm) |>
  ungroup() |>
  mutate(elapsed = yearsSinceStart - lowestYear,
         productivityIndex = min(elapsed)/elapsed)
