...
Then we model the density of all tree stems as a function of treatment to look for signs of self thinning.

> We could look at productivity and moose density (Laurents method?) here as well!

```{r}
q1_a <- lme4::lmer(data = modelData,
               Quantity ~ Treatment + (1|Region/LocalityCode))
```
...
There are only 15 sites in Hedmark:

```{r}
unique(modelData$LocalityName[modelData$Region=="SUSTHERB Hedmark-Akershus"])
```

There is one missing. It is still present in the full data set:

```{r}
unique(Data$LocalityName[Data$Region=="SUSTHERB Hedmark-Akershus"])
```
...
```{r}
q1_gamma <- lme4::glmer(data=modelData,
                      Quantity+0.1 ~ Treatment + (1|Region/LocalityCode),
                      family = Gamma(link="identity")) #I tried log and inverse link functions as well.
summary(q1_gamma)
```
...
The residuals are better, but not perfect. The link function does not affect them. I tried log and inverse as well.

```{r}
q1_poisson <- lme4::glmer(data=modelData,
                      Quantity ~ Treatment + (1|Region/LocalityCode),
...
q1_lognorm <- lmer(data=modelData,
               log(Quantity+1) ~ Treatment +(1|Region/LocalityCode))
summary(q1_lognorm)
```
