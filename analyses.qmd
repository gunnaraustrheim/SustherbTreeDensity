...
We will use a frequentist approach to mixed effects modelling with `lme4`. 
We have a random structure given by the sampling design. 
Our fixed structure is defined from theoretical expectations (a model set) 
and no model selection/reduction is done. 
Effect sizes are interpreted using the estimats and the errors, 
simulating a normal distribution for the estimates. P-values are not reported.

We will not look at the temporal trends, but rather focus on the last year. 
The last year with data is 2023, which is 15-11 years of data, 
...
It's a shame to cut 4 years with data, so I will just use the 2023 data for all 
regions. With only three regions it is possible to interpret the regional affect 
and duration effect combined.
...
Let's define that as the height class 7 (\>3m).
...
### Data aggregation
We can try to sum the number of stems per LocalityCode.
This variation was explained in the random structure in the models above.
If we aggregate this way, the data might fit easier.
```{r}
